...
async def _dispatch(
    ...
    if name == "browser_click_at":
        resp = await _safe_browser_post(http, f"/session/{browser_session_id}/click", json={"x": args["x"], "y": args["y"]})
        await _action("click_at", f"Clicked at ({int(args['x'])}, {int(args['y'])})")
        return "clicked"

    if name == "browser_mouse_move":
        resp = await _safe_browser_post(http, f"/session/{browser_session_id}/mouse_move", json={"x": args["x"], "y": args["y"]})
