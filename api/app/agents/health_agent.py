...
async def run(workspace_id: str, audience_user_id: str) -> None:
    with agent_run_context(
        ...
        _log.info("health_agent_start", workspace_id=workspace_id)
        async with AsyncSessionLocal() as session:
            ...
            for turn in range(40):
                ...
                if total_cost > COST_CEILING_USD:
                    _log.warning("health_agent_cost_ceiling", cost_usd=round(total_cost, 4))
                ...
                tool_calls_llm = getattr(msg, "tool_calls", None) or []
                tool_names_llm = [tc.function.name for tc in tool_calls_llm]
                _log.info(
                    "agent_llm_result",
                    turn=turn,
                    tool_calls=tool_names_llm,
            ...
            _log.info("health_agent_done", workspace_id=workspace_id)
